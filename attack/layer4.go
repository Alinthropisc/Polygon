...
// ampDispatch is the single AMP handler; it resolves the payload by method name.
func (l *Layer4) ampDispatch() {
	if payload, port := l.ampPayload(); payload != nil {
		l.amp(payload, port)
	}
}
